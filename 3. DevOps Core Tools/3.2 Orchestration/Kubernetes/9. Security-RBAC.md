...
```
Insecure access detected…
```

→ Try locally or use HTTPS.

---

# **7. Custom Resource Definitions (CRDs) – Clean Explanation**
...
| Deployments | `apps/v1`                 | “apps” API group   |
...
PSS does NOT enforce by itself — it must be applied using:
...
- **enforce** → Blocks pod
- **audit** → Allows pod but logs violation
- **warn** → Allows pod but shows warning

---

### ✔ **Important PSS Security Controls**
...
By default:
👉 **All traffic is allowed** (if no NetworkPolicy is defined)
...
- Block everything except traffic from frontend → backend
...
Encrypt secrets → Commit to Git → Decrypt only inside cluster.
