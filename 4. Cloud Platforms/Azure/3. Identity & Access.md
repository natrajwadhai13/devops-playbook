...
👉 [GitHub Repo link forIdentity & Access]()
...
# **Azure Resources, Resource Manager & Resource Groups**
...
* Any **Azure service** is a resource
  (VM, Storage Account, Database, VNet, App Service, etc.)
* Each resource has its **type, location, pricing, tags**.
* Resource = **smallest deployable unit** in Azure.

---

## **🔹 Resource Manager (ARM)**

* Azure’s **management and deployment engine**.
* ARM controls:
  ...
  * Tags
  * Policies
* ARM Templates = JSON files for **Infrastructure as Code (IaC)**.
...
* Group resources that share the **same lifecycle**
  (e.g., Web App + Database + VNet).
* Key points:

  * Deleting a Resource Group deletes **all resources inside**
  * RBAC can be applied at RG level
  * RG has one region, but resources inside can be in **different regions**
